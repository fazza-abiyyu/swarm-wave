<template>
  <div class="bg-white rounded-xl border border-gray-200 shadow-none p-6">
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-semibold text-gray-900">Particle Swarm Optimization (PSO) Results</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 h-64 overflow-y-auto" ref="logsContainer">
          <div v-if="!logs || logs.length === 0" class="text-gray-500 text-center">Run simulation to see PSO logs</div>
      ...
      <!-- Chart -->
      <div>
        <h3 class="text-lg font-medium text-gray-900 mb-2">PSO Convergence Chart</h3>
    ...
    <div class="mt-6">
      <div class="flex justify-between items-center mb-2">
        <h3 class="text-lg font-medium text-gray-900">
          PSO Final Assignment <span v-if="bestMakespan">(Best Makespan: {{ parseFloat(bestMakespan).toFixed(2) }}s)</span>
        </h3>
...
const initChart = () => {
  ...
  chart = new Chart(chartCanvas.value.getContext('2d'), {
    ...
    data: { labels: [], datasets: [
      { label: 'PSO Best Makespan', data: [], borderColor: 'rgb(239, 68, 68)', backgroundColor: 'transparent', tension: 0.1 },
      { label: 'Best Point', data: [], backgroundColor: 'rgb(239, 68, 68)', pointRadius: 5, showLine: false }
    ]},

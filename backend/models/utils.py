...
def generate_agen_default(jumlah_agen, agent_id_col="id"):
    ...
    return [{agent_id_col: f"Agent-{i + 1}"} for i in range(jumlah_agen)]
...
def fungsi_biaya_jadwal(jadwal, durasi_total):
    ...
    for penugasan in jadwal:
        id_agen = (
            ...
            or penugasan.get("id_agen")
            or penugasan.get("Agent_ID")
        )
        waktu_akhir = (
            penugasan.get("finish_time")
            or penugasan.get("waktu_selesai")
            or penugasan.get("Finish_Time")
            or 0
        )
...
def validasi_dependensi(daftar_tugas, task_id_col="id"):
    ...
    """
    id_tugas_set = {
        str(tugas.get(task_id_col, tugas.get("id", ""))) for tugas in daftar_tugas
    }
    masalah = []
